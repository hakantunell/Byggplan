...
async function ensureSchema(db: D1Database) {
  ...
  await db.prepare(`CREATE TABLE IF NOT EXISTS project_drawing_measurement_state(
...
function validDrawingState(value:unknown){
  return Boolean(value && typeof value==='object' && !Array.isArray(value));
}

export function registerProjectDocumentAnnotationRoutes(app:RouteApp){
  app.get('/api/project-drawing-measurements',async c=>{
    const projectId=c.req.query('projectId');if(!projectId)return c.json({ok:false,error:'projectId krävs.'},400);
    await ensureSchema(c.env.DB);
    const row=await c.env.DB.prepare('SELECT state_json,updated_at FROM project_drawing_measurement_state WHERE project_id=?').bind(projectId).first<any>();
    if(!row)return c.json({ok:true,state:{},updatedAt:null});
    let state:any={};try{state=JSON.parse(row.state_json||'{}')}catch{}
    return c.json({ok:true,state:validDrawingState(state)?state:{},updatedAt:row.updated_at||null});
  });

  app.put('/api/project-drawing-measurements',async c=>{
    await ensureSchema(c.env.DB);
    const body=await c.req.json<{projectId?:string;state?:unknown}>().catch(()=>({}));
    const projectId=typeof body.projectId==='string'?body.projectId.trim():'';
    if(!projectId||!validDrawingState(body.state))return c.json({ok:false,error:'Ogiltig mätdata.'},400);
    ...
    if(json.length>2_000_000)return c.json({ok:false,error:'Mätdatan är för stor.'},413);
    await c.env.DB.prepare(`INSERT INTO project_drawing_measurement_state(project_id,state_json,updated_at) VALUES(?,?,datetime('now'))
