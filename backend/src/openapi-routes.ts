...
const visionPage={name:'page',in:'path',required:true,schema:{type:'integer',minimum:1,maximum:20},description:'PDF-sida som ska renderas och bildtolkas'};
...
export function registerOpenApiRoutes(app:RouteApp){
  app.get('/api/openapi.json',c=>c.json({
    ...
    paths:{
      ...
      '/api/studio/governing-documents/{id}/layout-geometry':{get:{tags:['Diagnostik','Styrdokument'],summary:'Diagnostisera PDF-layoutens textgeometri',description:'Read-only diagnostik för kontrollplanslayout. Visar textobjektens x/y-position, bredd, höjd, teckenlängd och tilldelad kolumn för utvalda problemrader utan att ändra analys eller styrposter.',parameters:[documentId],responses:{'200':ok,'400':ok,'404':ok,'500':ok}}},
      ...
      '/api/studio/governing-documents/{id}/vision-conversion-diagnostics':{post:{tags:['Diagnostik','Styrdokument'],summary:'Diagnostisera hela PDF-bildtolkningen',description:'Renderar alla PDF-sidor och bildtolkar dem. Kan ta längre än normal HTTP-timeout för större eller långsammare dokument. För felsökning rekommenderas i första hand sidvis diagnostik.',parameters:[documentId],responses:{'200':ok,'400':ok,'404':ok,'500':ok,'503':ok}}},
      '/api/studio/governing-documents/{id}/vision-conversion-diagnostics/{page}':{post:{tags:['Diagnostik','Styrdokument'],summary:'Diagnostisera en PDF-sida med bildtolkning',description:'Renderar endast vald PDF-sida och kör samma Workers AI-bildtolkning som analysens pdf-vision-fallback. Returnerar exakt Markdown och tokenantal för sidan utan att ändra styrposter.',parameters:[documentId,visionPage],responses:{'200':ok,'400':ok,'404':ok,'500':ok,'503':ok}}},
