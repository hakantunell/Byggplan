...
app.post('/api/studio/import-tree', async c => {
  ...
  if (workAreaId) {
    ...
    ).bind(workAreaId, projectId, areaName, row?.next_order ?? 10));
  }

  const existingSectionOrder = await c.env.DB.prepare(
  ...
  let sectionOrder = existingSectionOrder?.max_order ?? 0;
  let sectionCount = 0;
  let taskCount = 0;
  let activityCount = 0;
