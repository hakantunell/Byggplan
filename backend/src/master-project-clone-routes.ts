...
export function registerMasterProjectCloneRoutes(app: RouteApp) {
  app.post('/api/studio/master-projects/:masterProjectId/create-project', async c => {
    ...
    const master = await c.env.DB.prepare(`
      ...
      WHERE id=?
    `).bind(masterProjectId).first<any>();
    ...
    const [areasResult, sectionsResult, tasksResult, activitiesResult] = await Promise.all([
      ...
      c.env.DB.prepare(`
        SELECT s.id,s.master_work_area_id,s.number,s.name,s.sort_order
        FROM master_work_sections s
        JOIN master_work_areas a ON a.id=s.master_work_area_id
        WHERE a.master_project_id=?
        ORDER BY a.sort_order,s.sort_order,s.id
      `).bind(masterProjectId).all(),
      c.env.DB.prepare(`
        SELECT t.id,t.master_work_section_id,t.title,t.description,t.sort_order
        FROM master_tasks t
        JOIN master_work_sections s ON s.id=t.master_work_section_id
        JOIN master_work_areas a ON a.id=s.master_work_area_id
        WHERE a.master_project_id=?
        ORDER BY a.sort_order,s.sort_order,t.sort_order,t.id
      `).bind(masterProjectId).all(),
      c.env.DB.prepare(`
        SELECT ac.id,ac.master_task_id,ac.title,ac.description,ac.activity_type,ac.required,ac.sort_order
        FROM master_activities ac
        JOIN master_tasks t ON t.id=ac.master_task_id
        JOIN master_work_sections s ON s.id=t.master_work_section_id
        JOIN master_work_areas a ON a.id=s.master_work_area_id
        WHERE a.master_project_id=?
        ORDER BY a.sort_order,s.sort_order,t.sort_order,ac.sort_order,ac.id
      `).bind(masterProjectId).all()
    ]);
    ...
    nextProjectOrder = Number(order?.next_order ?? 10);

    try {
      await c.env.DB.prepare(`
        INSERT INTO projects(id,name,property_designation,status,sort_order,created_at,updated_at)
        VALUES(?,?,?,'active',?,datetime('now'),datetime('now'))
      `).bind(projectId,name,propertyDesignation || null,nextProjectOrder).run();

      await c.env.DB.prepare(`
        INSERT INTO project_master_snapshots(project_id,master_project_id,master_project_code,master_project_version)
        VALUES(?,?,?,?)
      `).bind(projectId,master.id,master.code,Number(master.version)).run();

      for (const source of areas) {
        const id = crypto.randomUUID();
        areaMap.set(source.id,id);
        await c.env.DB.prepare(`
          INSERT INTO work_areas(id,project_id,name,description,status,sort_order,created_at,updated_at)
          VALUES(?,?,?,?,'todo',?,datetime('now'),datetime('now'))
        `).bind(id,projectId,source.name,source.number ? `Masterkod ${source.number}` : null,Number(source.sort_order)).run();
        await c.env.DB.prepare(`INSERT INTO project_master_node_links(id,project_id,entity_type,entity_id,master_entity_id) VALUES(?,?,'work_area',?,?)`)
          .bind(crypto.randomUUID(),projectId,id,source.id).run();
      }

      for (const source of sections) {
        const parentId = areaMap.get(source.master_work_area_id);
        if (!parentId) throw new Error(`Saknar arbetsområde för ${source.name}.`);
        const id = crypto.randomUUID();
        sectionMap.set(source.id,id);
        await c.env.DB.prepare(`
          INSERT INTO work_sections(id,work_area_id,name,description,status,sort_order,created_at,updated_at)
          VALUES(?,?,?,?,'todo',?,datetime('now'),datetime('now'))
        `).bind(id,parentId,source.name,source.number ? `Masterkod ${source.number}` : null,Number(source.sort_order)).run();
        await c.env.DB.prepare(`INSERT INTO project_master_node_links(id,project_id,entity_type,entity_id,master_entity_id) VALUES(?,?,'work_section',?,?)`)
          .bind(crypto.randomUUID(),projectId,id,source.id).run();
      }

      for (const source of tasks) {
        const parentId = sectionMap.get(source.master_work_section_id);
        if (!parentId) throw new Error(`Saknar arbetsavsnitt för ${source.title}.`);
        ...
        `).bind(id,parentId,section?.name || '',source.title,source.description || '',Number(source.sort_order)).run();
        await c.env.DB.prepare(`INSERT INTO project_master_node_links(id,project_id,entity_type,entity_id,master_entity_id) VALUES(?,?,'task',?,?)`)
          .bind(crypto.randomUUID(),projectId,id,source.id).run();
      }

      for (const source of activities) {
        const parentId = taskMap.get(source.master_task_id);
        if (!parentId) throw new Error(`Saknar moment för ${source.title}.`);
        const id = crypto.randomUUID();
        await c.env.DB.prepare(`
          INSERT INTO activities(id,task_id,title,description,activity_type,required,blocking,irreversible,sort_order)
          VALUES(?,?,?,?,?,?,0,0,?)
        `).bind(id,parentId,source.title,source.description || '',source.activity_type,Number(source.required ?? 1),Number(source.sort_order)).run();
        await c.env.DB.prepare(`INSERT INTO project_master_node_links(id,project_id,entity_type,entity_id,master_entity_id) VALUES(?,?,'activity',?,?)`)
          .bind(crypto.randomUUID(),projectId,id,source.id).run();
      }
    } catch (error) {
