...
export function registerGoverningDocumentConversionDiagnosticsRoutes(app:RouteApp){
  app.post('/api/studio/governing-documents/:id/conversion-diagnostics',async c=>{
    ...
    try{
      ...
      return c.json({
        ok:true,id,title:clean(document.title),documentType:clean(document.document_type),filename,contentType,
        sourceBytes:Number(document.size_bytes||0),conversionTokens:converted.tokens,convertedCharacters:markdown.length,convertedLines:lines.length,
        markdownPipeLines:pipeLines,markdownHeadingLines:headingLines,truncated,
        markdownPreview:markdown.slice(0,maxReturnedCharacters),markdownTail:truncated?markdown.slice(-4000):''
      });
    }catch(error){
  ...
  app.post('/api/studio/governing-documents/:id/vision-conversion-diagnostics',async c=>{
    const id=c.req.param('id');
    if(!c.env.AI||typeof c.env.AI.toMarkdown!=='function')return c.json({ok:false,error:'Workers AI dokumentkonvertering är inte konfigurerad.'},503);
    if(!c.env.BROWSER)return c.json({ok:false,error:'Browser Run-binding saknas.'},503);
    if(!c.env.FILES||typeof c.env.FILES.get!=='function')return c.json({ok:false,error:'Fillagringen är inte tillgänglig.'},503);
    const document=await c.env.DB.prepare(`SELECT d.id,d.title,d.document_type,d.source_filename,d.source_mime_type,
      f.object_key,f.original_name,f.content_type,f.size_bytes
      FROM governing_documents d JOIN governing_document_files f ON f.document_id=d.id WHERE d.id=?`).bind(id).first<any>();
    ...
    try{
      const filename=clean(document.original_name)||clean(document.source_filename)||'styrdokument';
      const contentType=clean(document.content_type)||clean(document.source_mime_type)||'application/octet-stream';
      if(contentType!=='application/pdf'&&!filename.toLowerCase().endsWith('.pdf'))return c.json({ok:false,error:'Visionsdiagnostiken är avsedd för PDF-dokument.'},400);
      const bytes=await object.arrayBuffer();
      const images=await renderPdfPages(c.env.BROWSER,bytes,20);
      const pages:any[]=[];let totalTokens=0,totalCharacters=0;
      for(let i=0;i<images.length;i++){
        const converted=await convertImagePage(c.env.AI,images[i],i+1);
        totalTokens+=converted.tokens;totalCharacters+=converted.text.length;
        pages.push({page:i+1,tokens:converted.tokens,characters:converted.text.length,lines:converted.text.split(/\r?\n/).length,markdown:converted.text});
      }
      const combinedMarkdown=pages.map(p=>`### Sida ${p.page}\n\n${p.markdown}`).join('\n\n');
      return c.json({
        ok:true,id,title:clean(document.title),filename,contentType,sourceBytes:Number(document.size_bytes||0),
        renderedPages:pages.length,totalTokens,totalCharacters,combinedCharacters:combinedMarkdown.length,pages,combinedMarkdown
      });
    }catch(error){
      console.error('Governing document vision conversion diagnostics failed',error);
      const detail=error instanceof Error?error.message:String(error);
      return c.json({ok:false,error:`Kunde inte diagnostisera bildtolkningen: ${detail}`},500);
    }
  });
}
