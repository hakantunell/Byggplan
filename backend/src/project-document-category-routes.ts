...
async function ensureCategory(db:D1Database){
  try{await db.prepare("ALTER TABLE project_documents ADD COLUMN category TEXT NOT NULL DEFAULT 'other'").run();}catch{}
}
...
export function registerProjectDocumentCategoryRoutes(app:RouteApp){
  app.get('/api/project-document-categories',async c=>{
    ...
    return c.json({ok:true,categories:(rows.results as any[]).map(row=>({id:String(row.id),category:row.category==='drawing'?'drawing':'other'}))});
  });

  app.put('/api/studio/project-documents/:id/category',async c=>{
    await ensureCategory(c.env.DB);
    const body=await c.req.json<{category?:string}>().catch(()=>({}));
    const category=body.category==='drawing'?'drawing':body.category==='other'?'other':'';
    if(!category)return c.json({ok:false,error:'Kategori måste vara drawing eller other.'},400);
