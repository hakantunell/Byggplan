...
async function fallbackMapping(c:any,cause:unknown){
 try{
  ...
  const itemRows=items.map(item=>{const ls=byItem.get(String(item.id))||[],handling_kind=kindFor(item);return{...item,handling_kind,handling_kinds:[handling_kind],mapped_activity_count:ls.length,mapped_activity_ids:ls.map(x=>String(x.activity_id)),mapped_activity_titles:ls.map(x=>String(x.activity_title||'')).filter(Boolean).join(' || ')||null,project_condition:false}});
  ...
  return c.json({ok:true,runtime:'mapping-v18-fallback',summary:{item_count,mapped_count,exception_count,project_condition_count:0,covered_count,uncovered_count,coverage_percent:item_count?Math.round(covered_count*100/item_count):100},documents:documentRows,items:itemRows,activities:activityRows,suggestions:{}},200);
