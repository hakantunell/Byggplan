...
async function ensureSection(db:D1Database,masterId:string,number:string,name:string){
 ...
 const areaNumber=number.split('.')[0];
 const area=await db.prepare('SELECT id FROM master_work_areas WHERE master_project_id=? AND number=?').bind(masterId,areaNumber).first<any>();
 if(!area)throw new Error(`Arbetsområde ${areaNumber} saknas i Master v2.`);
 const order=await db.prepare('SELECT COALESCE(MAX(sort_order),0)+10 AS n FROM master_work_sections WHERE master_work_area_id=?').bind(area.id).first<any>();
...
async function ensureTask(db:D1Database,sectionId:string,seed:TaskSeed){
 let task=await db.prepare('SELECT id FROM master_tasks WHERE master_work_section_id=? AND title=?').bind(sectionId,seed.title).first<any>();
 let taskId=task?.id?String(task.id):'';
 if(!taskId){const order=await db.prepare('SELECT COALESCE(MAX(sort_order),0)+10 AS n FROM master_tasks WHERE master_work_section_id=?').bind(sectionId).first<any>();taskId=crypto.randomUUID();await db.prepare('INSERT INTO master_tasks(id,master_work_section_id,title,description,sort_order) VALUES(?,?,?,?,?)').bind(taskId,sectionId,seed.title,'Generell kontrollpunkt för småhusprojekt. Aktiveras/kopplas när styrdokument kräver den.',Number(order?.n||10)).run();}
 let created=0;
 for(const activity of seed.activities){const exists=await db.prepare('SELECT id FROM master_activities WHERE master_task_id=? AND title=?').bind(taskId,activity.title).first();if(exists)continue;const order=await db.prepare('SELECT COALESCE(MAX(sort_order),0)+10 AS n FROM master_activities WHERE master_task_id=?').bind(taskId).first<any>();await db.prepare('INSERT INTO master_activities(id,master_task_id,title,description,activity_type,required,sort_order) VALUES(?,?,?,?,?,1,?)').bind(crypto.randomUUID(),taskId,activity.title,activity.description||'',activity.type,Number(order?.n||10)).run();created++;}
 return created;
}

export function registerMasterProjectV2UpgradeRoutes(app:RouteApp){
 app.post('/api/studio/master-projects/upgrade-fritidshus-v2',async c=>{
  const master=await c.env.DB.prepare("SELECT id,version FROM master_projects WHERE code='fritidshus-v2'").first<any>();
  if(!master)return c.json({ok:false,error:'Masterprojekt v2 finns inte ännu.'},404);
  let created=0;
  for(const seed of TASKS){const sectionId=await ensureSection(c.env.DB,String(master.id),seed.sectionNumber,seed.sectionName||'Generella kontroller');created+=await ensureTask(c.env.DB,sectionId,seed);}
  // Brandskydd är generellt för byggnaden och ska inte vara beroende av eldstadsmodulen.
  const fireSection=await ensureSection(c.env.DB,String(master.id),'80.15','Brandskydd');
  created+=await ensureTask(c.env.DB,fireSection,{sectionNumber:'80.15',title:'Dokumentera byggnadens brandskydd',activities:[
    {title:'Upprätta eller samla brandskyddsbeskrivning där det krävs',type:'document'},
  ...
  await c.env.DB.prepare('UPDATE master_projects SET version=CASE WHEN version<3 THEN 3 ELSE version END,updated_at=datetime(\'now\') WHERE id=?').bind(master.id).run();
  return c.json({ok:true,id:master.id,version:3,createdActivities:created});
 });
}
