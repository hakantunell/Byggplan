...
export function registerMasterProjectV2UpgradeRoutes(app:RouteApp){app.post('/api/studio/master-projects/upgrade-fritidshus-v2',async c=>{const master=await c.env.DB.prepare("SELECT id,version FROM master_projects WHERE code='fritidshus-v2'").first<any>();if(!master)return c.json({ok:false,error:'Masterprojekt v2 finns inte ännu.'},404);await ensureMetadataSchema(c.env.DB);let created=0;const mid=String(master.id);for(const seed of TASKS){const sid=await ensureSection(c.env.DB,mid,seed.sectionNumber,seed.sectionName||'Generella kontroller');created+=await ensureTask(c.env.DB,sid,seed)}
 const fireSection=await ensureSection(c.env.DB,mid,'80.15','Brandskydd');created+=await ensureTask(c.env.DB,fireSection,{sectionNumber:'80.15',title:'Dokumentera byggnadens brandskydd',activities:[
 ...
 created+=await addActivityToModuleTask(c.env.DB,mid,'fireplace','Installera eldstad och rökkanal',{title:'Spara intyg eller protokoll från sotarbesiktning av rökkanal och taksäkerhet',type:'administration',surface:'studio'});
 created+=await addActivityToModuleTask(c.env.DB,mid,'wetroom','Utför våtrum',{title:'Verifiera behörighet eller dokumentera vald våtrumsmetod',type:'administration',surface:'studio'});
 created+=await addActivityToModuleTask(c.env.DB,mid,'wetroom','Utför våtrum',{title:'Kontrollera att våtrummets ångtätningslösning inte ger olämpliga dubbla täta skikt',type:'check',applicability:'conditional',condition:'När våtrum med keramiska material ansluter mot yttervägg.'});
