...
export function registerMasterProjectV2UpgradeRoutes(app:RouteApp){app.post('/api/studio/master-projects/upgrade-fritidshus-v2',async c=>{
 ...
 await c.env.DB.prepare("UPDATE master_projects SET version=CASE WHEN version<6 THEN 6 ELSE version END,updated_at=datetime('now') WHERE id=?").bind(mid).run();return c.json({ok:true,id:mid,version:6,createdActivities:created});
})}
